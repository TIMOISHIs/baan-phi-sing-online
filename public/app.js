...
let audioCtx=null,masterGain=null,ambientMaster=null,sfxMaster=null,trackNodes=[],trackTimers=[];
...
const TURN_TRANSITION_MS=1900,TURN_ALERT_MS=120000;
let turnWarn30=false,turnWarn10=false;
...
function resetLocalRoom(){
  localStorage.removeItem(roomKey);localStorage.removeItem(legacyRoomKey);resetCardReveals();
}
...
function ensureAudio(){
  if(!audioCtx){
    ...
    const limiter=audioCtx.createDynamicsCompressor();limiter.threshold.value=-3;limiter.knee.value=0;limiter.ratio.value=20;
    masterGain.connect(limiter);limiter.connect(audioCtx.destination);
    ambientMaster=audioCtx.createGain();ambientMaster.gain.value=0;ambientMaster.connect(masterGain);
    sfxMaster=audioCtx.createGain();sfxMaster.gain.value=sfxVolume;sfxMaster.connect(masterGain);
  }
...
const LOFI_TRACKS={
  ...
  lofi_lantern:{bpm:86,roots:[146.83,130.81,116.54,110],melody:[0,7,null,10,3,null,2,null,7,null,3,0,null,10,7,null],swing:.10},
  lofi_rain:{bpm:66,roots:[98,82.41,73.42,87.31],melody:[0,null,null,7,10,null,3,null,2,null,7,null,3,null,0,null],swing:.18}
};
function startLofiTrack(key){
  const cfg=LOFI_TRACKS[key],ctx=ensureAudio();if(!cfg||!ctx)return;
  const bus=ctx.createGain(),filter=ctx.createBiquadFilter();bus.gain.value=.85;
  filter.type="lowpass";filter.frequency.value=1800;bus.connect(filter).connect(ambientMaster);trackNodes.push(bus,filter);
  const noise=ctx.createBuffer(1,Math.ceil(ctx.sampleRate*.18),ctx.sampleRate),data=noise.getChannelData(0);
  for(let i=0;i<data.length;i++)data[i]=Math.random()*2-1;
  const stepSeconds=60/cfg.bpm/4;let step=0,timer=null;
  function drum(kind){
    const now=ctx.currentTime,gain=ctx.createGain();gain.connect(bus);
    if(kind==="kick"){
      const osc=ctx.createOscillator();osc.frequency.setValueAtTime(120,now);osc.frequency.exponentialRampToValueAtTime(42,now+.13);
      gain.gain.setValueAtTime(.32,now);gain.gain.exponentialRampToValueAtTime(.0001,now+.24);osc.connect(gain);osc.start();osc.stop(now+.25);osc.onended=()=>{osc.disconnect();gain.disconnect()};
    }else{
      const src=ctx.createBufferSource(),f=ctx.createBiquadFilter();src.buffer=noise;f.type="highpass";f.frequency.value=kind==="hat"?6000:1400;
      const length=kind==="hat"?.045:.15;gain.gain.setValueAtTime(kind==="hat"?.035:.075,now);gain.gain.exponentialRampToValueAtTime(.0001,now+length);
      src.connect(f).connect(gain);src.start();src.stop(now+length);src.onended=()=>{src.disconnect();f.disconnect();gain.disconnect()};
    }
  }
  function beat(){
    if(timer!==null){const i=trackTimers.indexOf(timer);if(i>=0)trackTimers.splice(i,1)}
    if(!ambientEnabled||ambientTrack!==key)return;
    const n=step%16,root=cfg.roots[Math.floor(step/16)%cfg.roots.length];
    if(n===0||n===8||n===11)drum("kick");if(n===4||n===12)drum("snare");if(n%2===0)drum("hat");
    if(n===0){[0,3,7,10].forEach(v=>tone(root*2**(v/12),1.45,.022,"triangle",0,bus));tone(root/2,.6,.085,"sine",0,bus)}
    if(n===8)tone(root/2,.5,.065,"sine",0,bus);
    const note=cfg.melody[n];if(note!==null)tone(root*2*2**(note/12),.32,.025,"sine",0,bus);
    step++;timer=setTimeout(beat,stepSeconds*1000*(n%2===0?1+cfg.swing:1-cfg.swing));trackTimers.push(timer);
  }
  beat();
}
...
async function startAmbient(){
  ...
  ambientMaster.gain.setTargetAtTime(ambientVolume,ctx.currentTime,.08);
  startSelectedTrack();updateAmbientUI();
}
function stopAmbient(){
  ambientEnabled=false;localStorage.setItem("bpsAmbientV14","0");clearTrack();
  if(ambientMaster&&audioCtx)ambientMaster.gain.setTargetAtTime(0,audioCtx.currentTime,.08);
  updateAmbientUI();
}
function toggleAmbient(){ambientEnabled?stopAmbient():startAmbient()}
function setAmbientVolume(value){
  ambientVolume=Math.max(0,Math.min(3,Number(value)/100));localStorage.setItem("bpsAmbientVolV14",String(ambientVolume));
  if(ambientMaster&&audioCtx&&ambientEnabled)ambientMaster.gain.setTargetAtTime(ambientVolume,audioCtx.currentTime,.05);
}
function syncSoundSettings(){
  for(const [id,value] of [["masterVolume",masterVolume],["musicVolume",ambientVolume],["sfxVolume",sfxVolume]]){
    $("#"+id).value=String(Math.round(value*100));$("#"+id+"Value").textContent=`${Math.round(value*100)}%`;
  }
  $("#settingsMusicToggle").textContent=ambientEnabled?"ปิดเพลง":"เปิดเพลง";
}
function setSoundVolume(channel,value){
  ensureAudio();const level=Math.max(0,Math.min(3,Number(value)/100));
  if(channel==="music")setAmbientVolume(value);
  else if(channel==="master"){masterVolume=level;localStorage.setItem("bpsMasterVolV185",String(level));masterGain?.gain.setTargetAtTime(level,audioCtx.currentTime,.05)}
...
$("#masterVolume").oninput=e=>setSoundVolume("master",e.target.value);
$("#musicVolume").oninput=e=>setSoundVolume("music",e.target.value);
$("#sfxVolume").oninput=e=>setSoundVolume("sfx",e.target.value);
$("#settingsTestSound").onclick=playCardFlipSound;
...
function paintTurnTimer(){
  ...
  const left=Math.max(0,Math.ceil(((state?.game?.turnDeadline||0)-(serverClock.now()||0))/1000));const timer=$("#turnCountdown");if(timer){timer.textContent=`${Math.floor(left/60)}:${String(left%60).padStart(2,"0")}`;timer.classList.toggle("urgent",left<=10);timer.classList.toggle("warning",left<=30);}
...
function startTurnTimer(p){
  stopTurnTimer();if(!p||state?.phase!=="game")return;turnTimerPlayerId=p.id;turnWarn30=false;turnWarn10=false;turnTimerStart=performance.now()-Math.max(0,(serverClock.now()||0)-(state.game.turnStartedAt||serverClock.now()||0));
  const tick=()=>{
    if(state?.phase!=="game"||activePlayer()?.id!==turnTimerPlayerId){stopTurnTimer();return}
    const left=Math.max(0,(state.game.turnDeadline||(state.game.turnStartedAt+TURN_ALERT_MS))-(serverClock.now()||0));
    turnTimerProgress=Math.max(0,Math.min(1,1-left/TURN_ALERT_MS));paintTurnTimer();
    if(left<=30000&&!turnWarn30){turnWarn30=true;if(turnTimerPlayerId===myId())toast("เหลือเวลา 30 วินาที")}
    if(left<=10000&&!turnWarn10){turnWarn10=true;if(turnTimerPlayerId===myId())playTurnTimeWarningSound()}
...
function flushDiceQueues(kind=null){
  diceAnimating=false;const s=queuedState,p=queuedPrivateState,h=[...queuedHpEvents];queuedState=null;queuedPrivateState=null;queuedHpEvents=[];if(s)applyIncomingState(s);if(p){const before=mine;mine=p;queueGainedCards(before,p);persistSession();render()}const fire=()=>h.forEach(e=>queueHpFx(e.delta,e.reason||"HP เปลี่ยน"));if(kind==="ritual"&&h.length)setTimeout(fire,1900);else fire();
}
migrateLegacyRoom();

$("#createForm").addEventListener("submit",e=>{e.preventDefault();socket.emit("createRoom",{name:$("#createName").value,sessionToken,allowDuplicateCharacters:true})});
...
$("#skillBtn").onclick=()=>{
  ...
  if(t==="free_any_trap_curse"){
    const targets=state.players.filter(x=>!x.dead&&["คำสาป","กับดัก"].includes(roomAt(x.pos)?.type));if(!targets.length){toast("ยังไม่มีใครติดห้องคำสาป/กับดัก");return;}const box=document.createElement("div");box.innerHTML=`<p class="muted">เลือกตัวเองหรือเพื่อนที่ติดคำสาป/กับดัก • ใช้ 3 ธูป ช่วยทั่วกระดาน • ผู้ถูกช่วยทอยและเดินหนีทันที</p>`;targets.forEach(x=>{const b=document.createElement("button");b.className="modal-option";b.innerHTML=`<b>${x.id===myId()?"ตัวเอง · ":""}${x.name}</b><span>${roomAt(x.pos)?.name||""}</span>`;b.onclick=()=>{socket.emit("skill",{targetId:x.id});closeModal()};box.appendChild(b)});openModal("Skill — หมอธรรม",box);return;
  }
  if(t==="peek_four_choose_two"){
    socket.emit("skill",{side:"top"});return;
  }
  if(t==="remote_help"){
    const cards=(mine.amu||[]).filter(c=>c.category==="ช่วยเหลือ");if(!cards.length){toast("ไม่มีการ์ดช่วยเหลือบนมือ");return;}const box=document.createElement("div");box.innerHTML=`<p class="muted">เลือกเพื่อน แล้วเลือกการ์ดช่วยเหลือ 1 ใบ • ใช้ได้แม้อยู่คนละห้อง</p>`;const sel=document.createElement("select");sel.className="skill-target-select";state.players.filter(x=>x.id!==myId()&&!mine?.catHelp?.targets.includes(x.id)).forEach(x=>{const o=document.createElement("option");o.value=x.id;o.textContent=`${x.name} · ${x.dead?"เสียชีวิต":roomAt(x.pos)?.name||""}`;sel.appendChild(o)});box.appendChild(sel);cards.forEach(c=>{const b=document.createElement("button");b.className="modal-option";b.innerHTML=`${artMarkup(c.art,"option-art",c.name)}<b>${c.name}</b><span>${c.desc||""}</span>`;b.onclick=()=>{socket.emit("skill",{targetId:sel.value,uid:c.uid});closeModal()};box.appendChild(b)});openModal("Skill — แมวจร",box);return;
  }
};
...
function runCharacterRandom(){
  ...
  const claimed=new Set(state.settings?.allowDuplicateCharacters?[]:state.players.filter(p=>p.id!==myId()&&p.characterConfirmed&&p.characterKey).map(p=>p.characterKey));const eligible=(state.characters||[]).filter(c=>!claimed.has(c.key));if(!eligible.length){toast("ไม่มีตัวละครว่างแล้ว");return}
...
function centerOf(el){const r=el?.getBoundingClientRect?.();return r&&r.width&&r.height?{x:r.left+r.width/2,y:r.top+r.height/2}:null}
function trackAnimation(el,frames,duration){
  if(!el?.animate)return Promise.resolve();
  const a=el.animate(frames,{duration:reducedMotion()?1:duration,easing:"cubic-bezier(.22,.7,.25,1)",fill:"forwards"});motionObjects.add(a);
  return a.finished.catch(()=>{}).finally(()=>{a.cancel();motionObjects.delete(a)});
}
function rememberReveal(e){if(e.card?.uid&&(e.zone==="sacrifice"||["draw","gain","skill"].includes(e.reason))){if(e.playerId===myId()&&e.card.type!=="event")pendingHandCards.add(e.card.uid);seenRevealCards.add(e.card.uid);if(seenRevealCards.size>300)seenRevealCards.delete(seenRevealCards.values().next().value)}}
function startCardFlight(e){
  currentRevealEvent=e;
  if(e.zone!=="sacrifice"&&!["draw","gain","skill"].includes(e.reason))return;
  const card=$("#cardRevealCard"),to=centerOf(card),from=e.reason==="gain"?null:centerOf($(e.zone==="sacrifice"?"#sacDeckCount":"#amuDeckCount")?.closest(".deck-card"));
  if(from&&to)trackAnimation(card,[{transform:`translate(${from.x-to.x}px,${from.y-to.y}px) scale(.22) rotate(-8deg)`,opacity:.3},{transform:"none",opacity:1}],650);
  else trackAnimation(card,[{transform:"translateY(18px) scale(.92)",opacity:0},{transform:"none",opacity:1}],350);
}
function finishCardFlight(){
  const e=currentRevealEvent;currentRevealEvent=null;
  if(!e||e.card?.type==="event"||(e.zone!=="sacrifice"&&!["draw","gain","skill"].includes(e.reason)))return;
  const owned=e.playerId===myId()?(e.zone==="sacrifice"?mine?.sac:mine?.amu)?.some(c=>c.uid===e.card?.uid):true;
...
function queueGainedCards(before,after){
  if(!before||state?.phase!=="game")return;
  for(const [key,zone] of [["amu","amulet"],["sac","sacrifice"]])for(const card of after[key]||[]){
    if((before[key]||[]).some(c=>c.uid===card.uid)||seenRevealCards.has(card.uid))continue;
    enqueueCardReveal({card,zone,reason:"gain",playerId:myId()});
  }
}
async function animatePawnPath(p,path){
  if(!Array.isArray(path)||path.length<2||reducedMotion()){showMoveFx(p,p.pos);return}
  const seat=playerSeat(p),epoch=motionEpoch;
  if(movingSeats.has(seat))return;
  movingSeats.add(seat);render();
  const token=document.createElement("div");token.className="walking-pawn";token.innerHTML=pawnMarkup(p);token.querySelector(".pawn")?.classList.remove("pawn-in-transit");document.body.appendChild(token);motionObjects.add(token);
  document.querySelectorAll(`[data-pawn-seat="${seat}"]`).forEach(el=>{if(!token.contains(el))el.classList.add("pawn-in-transit")});
  const stepMs=Math.min(1500,3000/(path.length-1));
  try{for(let i=1;i<path.length;i++){
    if(epoch!==motionEpoch)break;
    const from=centerOf(document.querySelector(`[data-room-index="${path[i-1]}"]`)),to=centerOf(document.querySelector(`[data-room-index="${path[i]}"]`));if(!from||!to)break;
    playStepSound();await trackAnimation(token,[{transform:`translate(${from.x-20}px,${from.y-25}px)`},{transform:`translate(${(from.x+to.x)/2-20}px,${(from.y+to.y)/2-33}px)`},{transform:`translate(${to.x-20}px,${to.y-25}px)`}],stepMs);
  }}finally{token.remove();motionObjects.delete(token);movingSeats.delete(seat);document.querySelectorAll(`[data-pawn-seat="${seat}"]`).forEach(el=>el.classList.remove("pawn-in-transit"));if(epoch===motionEpoch){render();showMoveFx(p,p.pos)}}
}
function resetPresentationMotion(){motionEpoch++;for(const item of motionObjects){item.cancel?.();item.remove?.()}motionObjects.clear();movingSeats.clear();seenRevealCards.clear();pendingHandCards.clear();currentRevealEvent=null}
function ghostHealth(g){
  const need=g.ghost?.need||{};let total=0,done=0;
  for(const [color,n] of Object.entries(need)){total+=Math.max(0,n);done+=Math.min(Math.max(0,g.bossDone?.[color]||0),Math.max(0,n))}
  return {total,remaining:total-done,percent:total?Math.round((total-done)*100/total):0};
}
function renderGhostHealth(g){const h=ghostHealth(g),bar=$("#ghostHealthFill"),label=$("#ghostHealthLabel");if(!bar||!label)return;bar.style.width=`${h.percent}%`;label.textContent=`พลังผี ${h.percent}% · เหลือ ${h.remaining}/${h.total} สัญลักษณ์`;$("#ghostHealth").setAttribute("aria-valuenow",String(h.percent))}
function openSacrificeCard(c){
  const box=document.createElement("div");box.className="sacrifice-detail";
  const art=document.createElement("div");art.className="sacrifice-detail-art";art.innerHTML=artMarkup(c.art,"sacrifice-art-img",c.name)||"◆";
  const name=document.createElement("h2");name.textContent=c.name;const info=document.createElement("p");info.textContent=`${c.group||c.color} · ทำพิธีสำเร็จ +${c.boss} · คะแนนจบเกม ${c.end>=0?"+":""}${c.end}`;
  box.append(art,name,info);openModal(c.name,box);
}
let diceTimer=null,ritualTimer=null,diceAnimTimer=null,cardRevealBusy=false;
const cardRevealQueue=[];
let cardRevealTimer=null,cardRevealClosing=false,eventRevealOpen=false,cardRevealPreviousFocus=null;
function closeCardReveal(){
  if(!cardRevealBusy||cardRevealClosing)return;
  clearTimeout(cardRevealTimer);cardRevealClosing=true;eventRevealOpen=false;
  const box=$("#cardRevealFx");finishCardFlight();box.classList.add("card-out");
  cardRevealTimer=setTimeout(()=>{
    box.classList.add("hidden");box.classList.remove("card-out","event-reading");
    cardRevealBusy=false;cardRevealClosing=false;
    if(cardRevealPreviousFocus?.isConnected)cardRevealPreviousFocus.focus();
    showNextCardReveal();if(!cardRevealBusy){render();recoverMandatoryDecision();updateRollFocus()}
  },720);
}
function resetCardReveals(){
  resetPresentationMotion();
  clearTimeout(cardRevealTimer);cardRevealQueue.length=0;cardRevealBusy=false;cardRevealClosing=false;eventRevealOpen=false;
  $("#cardRevealFx").classList.add("hidden");$("#cardRevealFx").classList.remove("card-out","event-reading");
}
$("#cardRevealClose").onclick=closeCardReveal;
$("#cardRevealFx").addEventListener("keydown",e=>{if(eventRevealOpen&&e.key==="Tab"){e.preventDefault();$("#cardRevealClose").focus()}});
function enqueueCardReveal(e){
  rememberReveal(e);cardRevealQueue.push(e);
...
function showNextCardReveal(){
  ...
  cardRevealBusy=true;cardRevealClosing=false;
  const box=$("#cardRevealFx"),card=$("#cardRevealCard"),c=e.card||{};
  card.className="card-reveal-card";
  card.querySelector(".card-reveal-art").innerHTML=artMarkup(c.art,"card-art-img",c.name)||"✦";
  ...
  $("#cardRevealPlayer").textContent="";
  box.classList.remove("hidden","card-pop");void box.offsetWidth;box.classList.add("card-pop");startCardFlight(e);playCardFlipSound();
  eventRevealOpen=c.type==="event";
  box.classList.toggle("event-reading",eventRevealOpen);$("#cardRevealClose").classList.toggle("hidden",!eventRevealOpen);
  if(eventRevealOpen){
    cardRevealPreviousFocus=document.activeElement;$("#cardRevealClose").focus();hideRollFocus();
  }else cardRevealTimer=setTimeout(closeCardReveal,e.zone==="amulet"?2800:2200);
}
const DIE_ROTATIONS={1:[0,0],2:[0,-90],3:[-90,0],4:[90,0],5:[0,90],6:[0,180]};
function buildDie(el){
  const dots={1:[5],2:[1,9],3:[1,5,9],4:[1,3,7,9],5:[1,3,5,7,9],6:[1,3,4,6,7,9]};
  el.innerHTML=Object.entries(dots).map(([face,cells])=>`<div class="die-face face-${face}">${Array.from({length:9},(_,i)=>`<i class="die-pip ${cells.includes(i+1)?"on":""}"></i>`).join("")}</div>`).join("");
}
function settleDie(el,value){
  const rotation=DIE_ROTATIONS[value];if(!rotation)return;
  el.style.transform=`rotateX(${rotation[0]}deg) rotateY(${rotation[1]}deg)`;el.setAttribute("aria-label",`ลูกเต๋า ${value}`);el.dataset.value=String(value);
}
...
function showDiceFx(e){
  ...
  diceSettleTimer=setTimeout(()=>{
    settleDie(aEl,e.a);settleDie(bEl,e.b);total.textContent=`รวม ${e.total}`;
...
function openLogDetail(entry){
  const box=document.createElement("div");box.className="log-detail";
  const line=(text,tag="p")=>{const el=document.createElement(tag);el.textContent=text;box.appendChild(el);return el};
  line(new Date(entry.at).toLocaleTimeString("th-TH"),"small");line(entry.text,"h3");
  const d=entry.details||{};
  if(d.reason)line(`สาเหตุ: ${d.reason}`);
  if(d.affected)line(`ผู้รับผล: ${d.affected}`);
  if(d.actor)line(`เทิร์นของ: ${d.actor}${d.room?` • ${d.room}`:""}`);
  for(const c of d.changes||[])line(`${c.player} • ${c.field}: ${c.before} → ${c.after}`,"strong");
  for(const c of d.cards||[]){
    line(c.name,"h4");if(c.condition)line(`เงื่อนไข: ${c.condition}`);line(c.desc||"ดูผลที่บันทึกในเหตุการณ์ด้านบน");
    if(c.art){const img=document.createElement("img");img.src=c.art;img.alt=c.name;img.className="log-card-art";box.appendChild(img)}
  }
  if(d.curse!=null)line(`Curse ขณะบันทึก: ${d.curse}/6`,"small");
  $("#logDetailBody").replaceChildren(box);
  if(!$("#logDetailDialog").open)$("#logDetailDialog").showModal();
}
...
function renderGame(){
  ...
  $("#supportScore").textContent=mePublic()?.supportScore??0;
  ...
  $("#log").innerHTML="";
  state.log.forEach(x=>{const d=document.createElement("button");d.className="log-entry";d.type="button";d.textContent="• "+x.text;d.title="กดดูรายละเอียด";d.onclick=()=>openLogDetail(x);$("#log").appendChild(d)});
  $("#log").scrollTop=$("#log").scrollHeight;

  $("#board").innerHTML="";
  for(let i=0;i<9;i++){
    const r=roomAt(i),b=document.createElement("button");
    const style=r.boss?"boss":r.type==="คำสาป"?"curse":r.type==="กับดัก"?"trap":r.sac?"mystery":"safe";
    b.className=`room room-${style} ${r.boss?"boss":""}`;
    b.dataset.roomIndex=String(i);
    if(g.rescue){b.classList.add((g.rescue.legal||[]).includes(i)?"legal":"illegal");}
    else if(g.sanityDecision){
      const allowed=(g.movementPreview?.legal||[]).includes(i);b.classList.add(allowed?"legal":"illegal");
    }else if(g.mustMove||g.moveOptional)b.classList.add(g.legal.includes(i)?"legal":"illegal");
  ...
  $("#ghostCurse").textContent=ghost.curseText||"";
  ...
  let skillDisabled=!canAct||(g.actions<3&&!mine?.catHelp)||!skillType;
...
function renderPrivate(){
  ...
  $("#equip").innerHTML="";
  for(let i=0;i<(mine.char?.slots||0);i++){
    const c=mine.equip[i],b=document.createElement("button");b.className="equip";if(c)b.innerHTML=`${artMarkup(c.art,"equip-item-art",c.name)}<span>${c.name}</span>`;else b.textContent=`Equip ${i+1}`;b.disabled=!c;if(c)b.onclick=()=>openAmuletCard(c,{equipped:true});$("#equip").appendChild(b)
  }
  $("#amuHand").innerHTML="";
  ...
  $("#sacHand").innerHTML="";
  mine.sac.forEach(c=>{const b=document.createElement("button");b.className=`game-card ${c.color}`;b.innerHTML=`<b>${c.name}</b><small>${c.group||c.color}</small><div class="art sacrifice-art">${artMarkup(c.art,"sacrifice-art-img",c.name)}</div><small>ตีผี ${c.boss} · จบเกม ${c.end>=0?"+":""}${c.end}</small>`;if(pendingHandCards.has(c.uid))b.style.visibility="hidden";b.onclick=()=>openSacrificeCard(c);$("#sacHand").appendChild(b)});
  for(let i=mine.sac.length;i<7;i++){const empty=document.createElement("div");empty.className="sac-placeholder";empty.textContent=`SLOT ${i+1}`;$("#sacHand").appendChild(empty)}
}
...
function canUseAmulet(c,{equipped=false}={}){
  const g=state?.game,mp=mePublic();if(!g||g.rescue||!isMyTurn()||g.curseResolving||g.pendingRoomEffect||g.pendingRitual||eventRevealOpen||turnTransitionActive||diceAnimating||movingSeats.size)return false;
  if(mp?.dead)return mine?.char?.skillType==="self_revive"?false:false;
  if(equipped)return g.moved&&g.actions>=1;
  if(c.type==="sanity")return !!g.sanityDecision&&g.actions>=1;
  if(c.type==="equip")return g.moved&&g.actions>=1&&(mine?.equip?.length||0)<(mine?.char?.slots||0);
...
function renderTurnGuide({ap,mp,g,myRoom,pendingMine,canAct}){
  ...
  if(g.rescue){const who=state.players.find(p=>p.id===g.rescue.playerId)?.name||"ผู้ถูกช่วย";set(g.rescue.playerId===myId()?"หมอธรรมช่วยแล้ว — เดินหนีทันที":`รอ ${who} เดินหนี`,g.rescue.phase==="roll"?"ผู้ถูกช่วยกดทอยเดิน ไม่เสียธูปและไม่ใช้เทิร์นปกติ":"เลือกห้องที่ไฮไลท์ รับประกันออกได้อย่างน้อย 1 ห้องติดกัน",1);return}
  ...
  if(mp?.dead&&mine?.char?.skillType==="self_revive"){set("พ่อไกรยังกลับมาได้","HP 0 แต่สกิลพ่อไกรใช้ได้ตอนตาย • กด Skill 3🕯 เพื่อฟื้น HP +2",2);return}
...
function renderPendingRoomEffect(pending){
  ...
  if(pending.type==="negativeReaction"){
    ...
    const need=pending.count??Math.min(2,(pending.options||[]).filter(c=>!c.disabled).length);
    ...
    (pending.options||[]).forEach(c=>{const lab=document.createElement("label");lab.className="peek-card-choice";lab.innerHTML=`<input type="checkbox" ${c.disabled?"disabled":""} value="${c.uid}"><div>${c.disabled?'<div class="peek-card-art">✦</div>':artMarkup(c.art,"peek-card-art",c.name)}<b>${c.name}</b><small>${c.category||c.type||"Amulet"}</small><span>${c.desc||""}</span></div>`;grid.appendChild(lab)});box.appendChild(grid);
...
function renderResult(){
  ...
  $("#resultText").textContent=defeated?"ผู้เล่นทุกคน HP เหลือ 0 — เกมจบทันที":"คะแนนจากเงิน + เครื่องเซ่นบนมือ + คะแนนซัพพอร์ต";
  const rows=state.result?.rows||[];
  rows.forEach(p=>{const d=document.createElement("div");d.className="result-player "+(p.winner?"winner":"");d.innerHTML=`<span>${p.winner?"🏆 ":""}${p.name} · ${p.char}</span><b>${p.total} คะแนน ${p.xp!=null?`· +${p.xp} EXP`:""} <small style="display:block;color:#9c978c">เงิน ${p.ritual} + เครื่องเซ่น ${p.hand} + ซัพพอร์ต ${p.support||0}</small></b>`;$("#results").appendChild(d)});
