...
const q=s=>w.document.querySelector(s);assert.equal(q('#playerList').children.length,6);assert.equal(q('.bps-search').children.length,2);assert.equal(q('.bps-action-grid').children.length,5);q('#rollBtn').click();assert.equal(sent.at(-1)[0],'roll');q('#storyTab').click();assert.equal(q('#storyTab').getAttribute('aria-selected'),'true');q('#offeringsBtn').click();assert.equal(q('#offeringsDialog').open,true);q('#discardBtn').click();assert.equal(q('#discardDialog').open,true);assert.equal(q('#escapeBtn').classList.contains('roll-legacy'),false);const ids=[...w.document.querySelectorAll('[id]')].map(n=>n.id);assert.equal(ids.length,new Set(ids).size);dom.window.close();
